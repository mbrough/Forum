...
<?php
	...
	while ($row = mysql_fetch_array($rows)) {
		echo "To: <a href='readmsg.php?id=$row[chat_id]'>$row[fullname]</a>";
		
		$chatrow = mysql_fetch_array (ExecuteQuery ("SELECT * FROM chat WHERE chat_id=$row[chat_id] ORDER BY cdatetime ASC"));
		
		if ($chatrow) {
			echo "<br><br> $chatrow[message]<br>";
			echo "Sent: $chatrow[cdatetime]";
		}
	...
	while ($row = mysql_fetch_array($rows)) {
		echo "From: <a href='readmsg.php?id=$row[chat_id]'>$row[fullname]</a>";
		$chatrow = mysql_fetch_array (ExecuteQuery ("SELECT * FROM chat WHERE chat_id=$row[chat_id] ORDER BY cdatetime ASC"));
		
		if ($chatrow) 		{
			echo "<br><br> $chatrow[message]<br>";
			echo "Received: $chatrow[cdatetime]";
		}
		echo "<hr style='border-top:1px solid #c3c3c3; border-bottom:1px solid white'>";
	}
	require("footer.php")
